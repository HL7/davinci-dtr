...
If Adaptive Forms are being used, and a DTR service determines that prior authorization is both necessary and the requirements have been met, then the final question in the form **SHALL** be a question that asks the user if they would like the prior authorization identifier to be issued, and indicate the appropriate response options.  If the user's answer indicates a desire to have it issued, then the completed [`questionnaireResponse`](StructureDefinition-dtr-questionnaireresponse.html) from the payer will contain the issued Prior Authorization identifier in a [`coverage-information`](https://build.fhir.org/ig/HL7/davinci-crd/StructureDefinition-ext-coverage-information.html) extension (`satisfied-pa-id` element).
